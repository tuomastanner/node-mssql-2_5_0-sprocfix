...
(function() {
  var DECLARATIONS, JSON_COLUMN_ID, Pool, TYPES, Table, UDT, cast, createColumns, declare, getMssqlType, getTediousType, parameterCorrection, ref, tds, util, valueCorrection,
  ...
  module.exports = function(Connection, Transaction, Request, ConnectionError, TransactionError, RequestError) {
    ...
    TediousRequest = (function(superClass) {
      ...
      TediousRequest.prototype.query = function(command, callback) {
        var batchHasOutput, batchLastRow, columns, errorHandlers, errors, handleError, hasReturned, isJSONRecordset, jsonBuffer, recordset, recordsets, started;
        columns = {};
        recordset = [];
        recordsets = [];
        started = Date.now();
          ...
          return function(err, connection) {
            ...
            if (!err) {
              ...
              req.on('columnMetadata', function(metadata) {
                ...
                isJSONRecordset = false;
                if (_this.connection.config.parseJSON === true && metadata.length === 1 && metadata[0].colName === JSON_COLUMN_ID) {
              ...
              doneHandler = function(rowCount, more, rows) {
                ...
                if (isJSONRecordset) {
                  ...
                  jsonBuffer = null;
                  if (_this.verbose) {
                    _this._log(util.inspect(parsedJSON));
                    _this._log("---------- --------------------");
                  }
                  if (_this.stream) {
                    _this.emit('row', parsedJSON);
                  } else {
              ...
              req.on('row', function(columns) {
                ...
                if (isJSONRecordset) {
                  return jsonBuffer.push(columns[0].value);
                } else {
                  row = {};
      ...
      TediousRequest.prototype.execute = function(procedure, callback) {
        var columns, errorHandlers, errors, handleError, hasReturned, isJSONRecordset, jsonBuffer, recordset, recordsets, returnValue, started;
        columns = {};
        recordset = [];
        recordsets = [];
        returnValue = 0;
          ...
          return function(err, connection) {
            ...
            if (!err) {
              ...
              req.on('returnValue', function(parameterName, value, metadata) {
				...
				value = value === tds.TYPES.Null ? null : value;
				if (parameterName) _this.parameters[parameterName].value = value; 
                return value;
              });
              ref1 = _this.parameters;
              for (name in ref1) {
